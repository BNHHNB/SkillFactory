...
class BoardUsedException(BoardException):
    def __str__(self):
        return 'В эту клетку уже стреляли! Придумай что-нибудь новое!'
            ...
            for dx, dy in around:
                ...
                if not self.out(curr_dot) and curr_dot not in self.busy:
                    if visible:      # видимость контура
                        self.field[curr_dot.x][curr_dot.y] = 'T'
...
class Game:
    ...
    def greet():
        ...
        print('Приветсвую в игре морской бой!')
        print('Сейчас я обьясню тебе правила игры:')
        print('Все просто есть поле форматом 6x6 \nтвоя задача указать координаты выстрела')
        print('Куда стреляем пишем цифрами, формат ввода: x y где:')
        print(' x - номер строки  ')
        print(' y - номер столбца ')
        print('-------------------')

# вывод двух досок рядом
